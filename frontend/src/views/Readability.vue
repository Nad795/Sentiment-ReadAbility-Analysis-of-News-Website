...
const updateAnimation = () => {
  const target = fleschData[0].score
...
// === Expandable Sections ===
const expanded = ref(fleschData.slice(1).map(() => true)) // semua terbuka saat awal
const toggleExpand = (i) => {
  expanded.value[i] = !expanded.value[i]
}

// Animasi Bar di Expandable Sections
// Tambahkan setelah expanded
const progressWidths = ref(fleschData.slice(1).map(() => 0))

onMounted(() => {
  fleschData.slice(1).forEach((item, i) => {
    setTimeout(() => {
